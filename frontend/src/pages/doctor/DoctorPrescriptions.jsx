  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button icon={Plus} onClick={() => setWriteOpen(true)} disabled={appointments.length === 0}>Write Prescription</Button>
      </div>

      {loading ? (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">{Array.from({ length: 2 }).map((_, i) => <CardSkeleton key={i} />)}</div>
      ) : prescriptions.length === 0 ? (
        <Card className="text-center py-12"><p className="text-ink-400 text-sm">You haven't issued any prescriptions yet.</p></Card>
      ) : (
          ...
          {prescriptions.map((rx) => (
            <Card key={rx.id} hover className="animate-fade-up">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="font-display font-semibold text-ink-900">{rx.patient?.user?.name}</p>
              ...
              <div className="flex items-center gap-1.5 text-xs text-ink-500 mb-4"><CalendarDays className="w-3.5 h-3.5" />{rx.issuedDate?.slice(0, 10)}</div>
      ...
      <Modal open={writeOpen} onClose={() => setWriteOpen(false)} title="Write New Prescription" size="lg">
        <form id="write-rx-form" onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
            <Field label="Appointment (Patient)" className="sm:col-span-2">
              <Select name="appointmentId" defaultValue={appointments[0]?.id || ""}>
                {appointments.map((a) => (
                  <option key={a.id} value={a.id}>
                    {a.patient?.user?.name} — {a.apptDate?.slice(0, 10)} {a.apptTime}
                  </option>
                ))}
              </Select>
            </Field>
            <Field label="Diagnosis" className="sm:col-span-2"><Input name="diagnosis" required placeholder="e.g. Acute Bronchitis" /></Field>
          </div>
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <Button type="submit" form="write-rx-form" loading={saving}>Issue Prescription</Button>
        </div>
      </Modal>
    </div>
  );
